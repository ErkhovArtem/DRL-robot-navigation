...
Paths:
- All paths (models, logs, configs, buffers) default to src3/ directory
- Config file lookup prioritizes src3/configs/, then src2/configs/, then src/configs/
...
from policy.TD3.TD3 import TD3
from policy.PPO.PPO_agent import PPO
...
class CurriculumManager:
    ...
    def apply_current_level(self, reward_weights, obstacle_params, policy_config=None, replay_buffer_config=None):
        ...
        Args:
            ...
            policy_config (dict): Policy (SAC/TD3/PPO) config to update in-place (optional)
        ...
        for i in range(self.current_level_idx + 1):
            ...
            # Update SAC/TD3/PPO parameters
            if policy_config is not None:
                if 'sac' in level_data:
                    policy_config.update(level_data['sac'])
                elif 'td3' in level_data:
                    policy_config.update(level_data['td3'])
                elif 'ppo' in level_data:
                    policy_config.update(level_data['ppo'])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", type=str, default="SAC", choices=["SAC", "TD3", "PPO"], help="Reinforcement learning algorithm to use")
    ...
    # Build potential paths to try (prioritize src3, then src2, then src)
    potential_paths = [
        config_file_path,  # Relative to current directory (e.g., configs/g1.yaml from src3/)
        Path.cwd() / config_file_path,  # Explicitly from current directory
        PROJECT_ROOT / "src3" / "configs" / config_file_path.name,  # src3/configs/filename.yaml
        PROJECT_ROOT / "src2" / "configs" / config_file_path.name,  # src2/configs/filename.yaml
        PROJECT_ROOT / "src" / "configs" / config_file_path.name,  # src/configs/filename.yaml
        PROJECT_ROOT / "src3" / config_file_path,  # src3/configs/g1.yaml (if relative path)
        PROJECT_ROOT / "src2" / config_file_path,  # src2/configs/g1.yaml (if relative path)
        PROJECT_ROOT / "src" / config_file_path,  # src/configs/g1.yaml (for backward compatibility)
    ]
    ...
    curriculum_config = {}
    curriculum_path = PROJECT_ROOT / "src3" / "configs" / "curriculum.yaml"
    ...
    print(f"Policy ({args.policy}): {policy_freq:.1f} Hz (decimation={sac_decimation})")
    ...
    # Extract policy configuration (SAC or TD3) based on --policy argument
    policy_type = args.policy
    policy_config = config.get(policy_type.lower(), {}).copy()  # Make a copy to allow curriculum updates
    if not policy_config:
        print(f"Warning: Configuration for policy '{policy_type}' not found in {args.config}. Using defaults.")
    ...
    # Shared parameters extracted from the selected policy's configuration (after curriculum updates)
    ...
    if policy_type == "SAC":
        ...
        # Defaults for other policies (e.g. TD3) to prevent NameErrors later
        sac_config = {}
        alpha_lr = 0.0001
    ...
    model_dir = PROJECT_ROOT / "src3" / "models"
    model_name = args.policy.lower()  # "sac" or "td3" based on --policy argument
    buffer_dir = PROJECT_ROOT / "src3" / "buffer"
    ...
    base_log_dir = Path(args.log_dir) if args.log_dir.startswith("/") else PROJECT_ROOT / "src3" / args.log_dir
    ...
    # Note: state_dim is base dimension, SAC/TD3 will compute stacked dimension internally
    print(f"Algorithm: {args.policy}")
    ...
    if args.policy == "SAC":
        ...
        # TD3 hyperparameters from config
        td3_config = config.get("td3", {})
        policy_noise = td3_config.get("policy_noise", 0.2)
        noise_clip = td3_config.get("noise_clip", 0.5)
        exploration_noise = td3_config.get("exploration_noise", 0.1)
        policy_frequency = td3_config.get("policy_frequency", 2)
        ...
        agent = TD3(
            ...
            discount=discount,
            tau=critic_tau,
            learning_rate=actor_lr, # Using same LR as SAC for now
        ...
        clip_vloss = ppo_config.get("clip_vloss", True)
        norm_adv = ppo_config.get("norm_adv", True)
        update_epochs = ppo_config.get("update_epochs", 10)
        num_minibatches = ppo_config.get("num_minibatches", 32)
    ...
    try:
        ...
        if use_mjx:
            ...
            while episode < max_episode:
                ...
                robot_spawn_pos = spawn_generator.sample_spawn_point(z_height=0.793)
                ...
                while step_count < max_steps and not done:
                    ...
                    try:
                        robot_dof_pos = d.qpos[7:7+num_actions]
                        robot_dof_vel = d.qvel[6:6+num_actions]
                        tau = pd_control(target_dof_pos, robot_dof_pos, kps, np.zeros_like(kds), robot_dof_vel, kds)
                        d.ctrl[:] = tau
                        mujoco.mj_step(m, d)
                    except Exception as e:
                    ...
                    if step_count % control_decimation == 0:
                        ...
                        if step_count % (control_decimation * sac_decimation) == 0:
                            ...
                            # SAC/TD3 outputs in [-1, 1], apply scaling from config
